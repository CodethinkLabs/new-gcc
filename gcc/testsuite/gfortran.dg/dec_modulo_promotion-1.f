!{ dg-do run }
!{ dg-options "-Wconversion-extra -fdec" }
!
...
!
      program test_small_type_promtion
        ...
        if (modulo(r4, b1).ne.4.0_4) stop 73 ! { dg-warning "Conversion from" }
        if (kind(modulo(r4, b1)).ne.4) stop 74 ! { dg-warning "Conversion from" }
        write(buffer,*) modulo(r4, b1) ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
        if (buffer(1:3).ne."4.0") stop 75
        if (modulo(a1, r4).ne.0.0_4) stop 76 ! { dg-warning "Conversion from" }
        if (kind(modulo(a1, r4)).ne.4) stop 77 ! { dg-warning "Conversion from" }
        write(buffer,*) modulo(a1, r4) ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
        if (buffer(1:3).ne."0.0") stop 78
	!
        if (modulo(r8, b2).ne.0.0_8) stop 79 ! { dg-warning "Conversion from" }
        if (kind(modulo(r8, b2)).ne.8) stop 80 ! { dg-warning "Conversion from" }
        write(buffer,*) modulo(r8, b2) ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
        if (buffer(1:3).ne."0.0") stop 81
        if (modulo(a2, r8).ne.4.0_8) stop 82 ! { dg-warning "Conversion from" }
        if (kind(modulo(a2, r8)).ne.8) stop 83 ! { dg-warning "Conversion from" }
        write(buffer,*) modulo(a2, r8) ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
