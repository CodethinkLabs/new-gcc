!{ dg-do run }
!{ dg-options "-cpp -Wconversion-extra -fdec" }
!
...
!
      program test_small_type_promtion
        ...
        !
        if (dim(r4, b1).ne.0.0_4) stop 73   ! { dg-warning "Conversion from" }
        if (kind(dim(r4, b1)).ne.4) stop 74 ! { dg-warning "Conversion from" }
        write(buffer,*) dim(r4, b1)         ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
        if (buffer(1:3).ne."0.0") stop 75
        if (dim(a1, pr4).ne.99.0_4) stop 76  ! { dg-warning "Conversion from" }
        if (kind(dim(a1, pr4)).ne.4) stop 77 ! { dg-warning "Conversion from" }
        write(buffer,*) dim(a1, pr4)         ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
        if (buffer(1:4).ne."99.0") stop 78
        !
        if (dim(r8, b2).ne.0.0_8) stop 79   ! { dg-warning "Conversion from" }
        if (kind(dim(r8, b2)).ne.8) stop 80 ! { dg-warning "Conversion from" }
        write(buffer,*) dim(r8, b2)         ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
        if (buffer(1:3).ne."0.0") stop 81
        if (dim(a2, pr8).ne.21.0_8) stop 82  ! { dg-warning "Conversion from" }
        if (kind(dim(a2, pr8)).ne.8) stop 83 ! { dg-warning "Conversion from" }
        write(buffer,*) dim(a2, pr8)         ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
