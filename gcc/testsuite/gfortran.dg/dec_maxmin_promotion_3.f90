...
program test
  ...
  if (kind(min(1_1, 2_2, 4_4, 8.0_16)).ne.16) stop 21 ! { dg-error "'a4' argument" }
  write(buffer,*) min(1_1, 2_2, 4_4, 8.0_16)          ! { dg-error "'a4' argument" }
  buffer = adjustl(buffer)
  if (buffer(1:3).ne."1.0") stop 22
  if (kind(min(a1, a2, b4, a8)).ne.8) stop 23 ! { dg-error "'a3' argument" }
  write(buffer,*) min(a1, a2, b4, a8)         ! { dg-error "'a3' argument" }
  buffer = adjustl(buffer)
