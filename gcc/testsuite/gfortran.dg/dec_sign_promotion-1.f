...
!
      program test_small_type_promtion
        ...
        if (sign(r4, p1).ne.4.0_4) stop 73 ! { dg-warning "Conversion from" }
        if (kind(sign(r4, p1)).ne.4) stop 74 ! { dg-warning "Conversion from" }
        write(buffer,*) sign(r4, p1) ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
        if (buffer(1:3).ne."4.0") stop 75
        if (sign(a1, pr4).ne.1.0_4) stop 76 ! { dg-warning "Conversion from" }
        if (kind(sign(a1, pr4)).ne.4) stop 77 ! { dg-warning "Conversion from" }
        write(buffer,*) sign(p1, pr4) ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
