...
!
      program test_small_type_promtion
        ...
        if (mod(r4, b1).ne.4.0_4) stop 73 ! { dg-warning "Conversion from" }
        if (kind(mod(r4, b1)).ne.4) stop 74 ! { dg-warning "Conversion from" }
        write(buffer,*) mod(r4, b1) ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
        if (buffer(1:3).ne."4.0") stop 75
        if (mod(a1, r4).ne.0.0_4) stop 76 ! { dg-warning "Conversion from" }
        if (kind(mod(a1, r4)).ne.4) stop 77 ! { dg-warning "Conversion from" }
        write(buffer,*) mod(a1, r4) ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
        if (buffer(1:3).ne."0.0") stop 78
	!
        if (mod(r8, b2).ne.0.0_8) stop 79 ! { dg-warning "Conversion from" }
        if (kind(mod(r8, b2)).ne.8) stop 80 ! { dg-warning "Conversion from" }
        write(buffer,*) mod(r8, b2) ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
        if (buffer(1:3).ne."0.0") stop 81
        if (mod(a2, r8).ne.4.0_8) stop 82 ! { dg-warning "Conversion from" }
        if (kind(mod(a2, r8)).ne.8) stop 83 ! { dg-warning "Conversion from" }
        write(buffer,*) mod(a2, r8) ! { dg-warning "Conversion from" }
        buffer = adjustl(buffer)
